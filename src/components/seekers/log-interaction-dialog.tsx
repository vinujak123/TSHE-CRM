...
export function LogInteractionDialog({ seeker, open, onOpenChange, onSuccess }: LogInteractionDialogProps) {
  ...
  const onSubmit = async (data: InteractionFormData) => {
    setIsLoading(true)
    try {
      const response = await fetch(`/api/seekers/${seeker.id}/interactions`, {
        method: 'POST',
        headers: {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>Log Interaction - {seeker.fullName}</DialogTitle>
        </DialogHeader>
